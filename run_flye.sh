...
# run_flye.sh [input read file] [Filename extension of the input read file (fastq/fastq.gz/etc)] [probable genome size] [number of polishing rounds] [number of threads] [error level of reads]
# Change the number of threads for your computer.
# Copyright (C) 2022-2023 Yu Wan <wanyuac@126.com>
# Licensed under the GNU General Public Licence version 3 (GPLv3) <https://www.gnu.org/licenses/>.
# First version: 18 Dec 2022; latest update: 15 July 2023
...
if [ "$m" == "hq" ]; then
    ...
    flye --nano-hq $r --threads $t --out-dir ./tmp --iterations $p --genome-size $g  # --subassemblies is incompatible with --nano-raw or --nano-corr
else
    echo "Assembling regular (error rate <15%) ONT reads $r using Flye v$v (polish: ${p}) with $t threads"
    flye --nano-raw $r --threads $t --out-dir ./tmp --iterations $p --genome-size $g
fi
