...
# First version: 7 November 2024; last update: 7 November 2024
...
display_usage() {
    ...
    Example useage:
      ~/bin/Assembly_toolkit/trycycler_run_medaka.sh -d=2_clusters -m=r941_min_hac_g507 -s=isolate1 -t=16 > trycycler_run_medaka.log
...
echo "[$(date)] Concatenate consensus sequences"
cat "$parental_dir"/cluster_*/7_final_consensus.fasta > "$parental_dir/1_${sample_name}_trycycler_consensus_raw.fasta"  # Unpolished
cat "$parental_dir"/cluster_*/8_medaka.fasta > "$parental_dir/2_${sample_name}_trycycler_consensus_medaka.fasta"  # Medaka-polished
